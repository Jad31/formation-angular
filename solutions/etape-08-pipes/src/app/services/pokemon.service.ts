...
import { PokeApiListResponse, PokeApiPokemon, Pokemon } from '../models/pokemon.model';
...
export class PokemonService {
  ...
  readonly filteredPokemons = computed(() => {
    ...
    if (search) {
      pokemons = pokemons.filter((p) => p.name.toLowerCase().includes(search));
    }
  ...
  loadPokemons(limit = 151): void {
    this._loading.set(true);
    this._error.set(null);

    this.http.get<PokeApiListResponse>(`${this.apiUrl}/pokemon?limit=${limit}`).subscribe({
      next: (response) => {
        const requests = response.results.map((p) => this.http.get<PokeApiPokemon>(p.url));
  ...
  searchPokemon(name: string): void {
    ...
    this.http.get<PokeApiPokemon>(`${this.apiUrl}/pokemon/${name.toLowerCase()}`).subscribe({
      next: (pokemon) => {
